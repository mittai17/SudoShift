...
export function FloatingToolbar() {
  ...
  return (
    <Panel position="bottom-center" className="mb-6">
      <div className="flex items-center bg-[#13141c] border border-[#2a2b36] rounded-2xl shadow-2xl p-1.5 relative">
        <span className="px-3 text-xs font-semibold text-gray-500 border-r border-[#2a2b36] select-none">
          {selectedNodes.length} {selectedNodes.length === 1 ? 'Node' : 'Nodes'}
        </span>

        <div className="flex items-center px-1.5 space-x-1">
          {/* Edit is conceptual here since nodes have inline inputs, but good for UX consistency */}
          {singleNode && (
            <button className="p-2 text-gray-400 hover:text-white hover:bg-[#1a1b23] rounded-xl transition-colors group">
          ...
          <button onClick={handleDuplicate} className="p-2 text-gray-400 hover:text-white hover:bg-[#1a1b23] rounded-xl transition-colors group">
          ...
          <button onClick={() => setShowColorPicker(!showColorPicker)} className={`p-2 rounded-xl transition-colors group ${showColorPicker ? 'bg-indigo-500/20 text-indigo-400' : 'text-gray-400 hover:text-white hover:bg-[#1a1b23]'}`}>
          ...
          <button onClick={handleDelete} className="p-2 text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-xl transition-colors group">
