...
import { CheckSquare, Plus, GripVertical, Calendar, User, ArrowUp, ArrowDown } from 'lucide-react';
...
const EventChecklistBody = ({ task, updateTask }: any) => {
  ...
  const updateItems = (newItems: ChecklistItem[]) => updateTask({ items: newItems, progress: newItems.length ? Math.round((newItems.filter(i => i.done).length / newItems.length) * 100) : 0 });
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between text-xs text-gray-400 mb-1">
        <span className="font-mono">{items.filter(i => i.done).length}/{items.length} Completed</span>
        <button onClick={bulkComplete} className="text-amber-500 hover:underline">Bulk Complete</button>
      </div>
      <div className="h-1.5 w-full bg-[#13141c] rounded-full overflow-hidden border border-[#2a2b36]">
        <div className={`h-full transition-all ${isAutoCompleted ? 'bg-emerald-500' : 'bg-amber-500'}`} style={{ width: `${progress}%` }} />
      </div>
        ...
        {items.map((item, index) => (
          <div key={item.id} className={`flex flex-col gap-1.5 bg-[#2a2b36]/30 p-2 rounded-xl border ${item.done ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-[#2a2b36] hover:border-[#3f3f46]'}`}>
            ...
            <div className="flex items-center gap-1.5 pl-6">
              <input 
                type="date" className="text-[10px] bg-[#13141c] text-gray-400 border border-[#2a2b36] rounded px-1 py-0.5 focus:outline-none focus:border-amber-500 w-24"
      ...
      <button onClick={addItem} className="flex items-center text-xs text-amber-500 hover:text-amber-400 py-1">
        <Plus className="w-3.5 h-3.5 mr-1" /> Add Item
      </button>

      {isAutoCompleted && (
        <div className="text-[10px] text-center text-emerald-400 bg-emerald-500/10 py-1 rounded border border-emerald-500/20 font-bold tracking-wide">
          AUTO-COMPLETED
        </div>
      )}
    </div>
