...
const DiscordNodeBody = ({ task, updateTask }: any) => {
  const [isFetching, setIsFetching] = useState(false);
  const [error, setError] = useState('');
  const [result, setResult] = useState('');
  const [apiKey, setApiKey] = useState(() => localStorage.getItem('discord_bot_token') || '');
  

  
  const actionType = task.actionType || 'send';
  ...
  const handleFetch = async () => {
    ...
    try {
      const res = await fetch('/api/integrations/discord', {
        ...
        body: JSON.stringify({ action: actionType, apiKey, token: apiKey, channelId, message }),
      });
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            localStorage.setItem('discord_bot_token', e.target.value);
          }}
        />
      </div>
      ...
      <div className="flex flex-wrap gap-2 text-xs">
         <button 
            onClick={() => updateTask({ actionType: 'send' })}
            className={`flex-1 min-w-[80px] flex items-center justify-center py-1.5 rounded-lg border transition-colors ${actionType === 'send' ? 'bg-[#2a2b36] text-white border-[#3f3f46]' : 'bg-[#13141c] text-gray-400 border-transparent hover:bg-[#1a1b23]'}`}
         >
            send
         </button>
         <button 
      ...
      <div className="flex flex-col gap-2">
         <div className="flex items-center bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
            ...
            <input 
               ...
               className="w-full text-xs bg-transparent focus:outline-none text-gray-300"
               value={channelId} onChange={(e) => updateTask({ channelId: e.target.value })} 
            />
         </div>
          ...
          {result && (
            <button
              onClick={() => {
                ...
                a.download = 'DiscordNode-result.json';
                a.click();
              }}
