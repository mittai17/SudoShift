...
const GitHubNodeBody = ({ task, updateTask }: any) => {
  ...
  const [apiKey, setApiKey] = useState(() => localStorage.getItem('github_api_key') || '');
  ...
  React.useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data?.type === 'GITHUB_AUTH_SUCCESS') {
        ...
        localStorage.setItem('github_api_key', event.data.token);
  ...
  const handleLogin = () => {
    ...
    window.open('/api/auth/github/login', 'GitHub Auth', `width=${width},height=${height},left=${left},top=${top}`);
  };
  ...
  const handleFetch = async () => {
    ...
    try {
      const res = await fetch('/api/integrations/github', {
        ...
        body: JSON.stringify({ action: actionType, apiKey, owner, repo, title, body }),
      });
  ...
  return (
    <div className="space-y-3">
      ...
      {!apiKey ? (
        ...
        >
          {isAuthenticating ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <SiGithub className="w-4 h-4 mr-2" />}
        ...
        <div className="flex items-center gap-2 bg-[#13141c] border border-green-500/30 rounded-lg p-2 transition-colors">
          ...
          <button 
            onClick={() => { setApiKey(''); localStorage.removeItem('github_api_key'); }}
      ...
      <div className="flex flex-wrap gap-2 text-xs">
         ...
         <button 
            onClick={() => updateTask({ actionType: 'create-issue' })}
            className={`flex-1 min-w-[80px] flex items-center justify-center py-1.5 rounded-lg border transition-colors ${actionType === 'create-issue' ? 'bg-[#2a2b36] text-white border-[#3f3f46]' : 'bg-[#13141c] text-gray-400 border-transparent hover:bg-[#1a1b23]'}`}
         >
            create-issue
         </button>
         <button 
            onClick={() => updateTask({ actionType: 'prs' })}
            className={`flex-1 min-w-[80px] flex items-center justify-center py-1.5 rounded-lg border transition-colors ${actionType === 'prs' ? 'bg-[#2a2b36] text-white border-[#3f3f46]' : 'bg-[#13141c] text-gray-400 border-transparent hover:bg-[#1a1b23]'}`}
         >
            prs
         </button>
         <button 
            onClick={() => updateTask({ actionType: 'repo' })}
            className={`flex-1 min-w-[80px] flex items-center justify-center py-1.5 rounded-lg border transition-colors ${actionType === 'repo' ? 'bg-[#2a2b36] text-white border-[#3f3f46]' : 'bg-[#13141c] text-gray-400 border-transparent hover:bg-[#1a1b23]'}`}
         >
            repo
         </button>
      </div>
      ...
      <div className="flex flex-col gap-2">
         <div className="flex items-center bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
            ...
            <input 
               type="text" placeholder="Owner/Organization..."
               className="w-full text-xs bg-transparent focus:outline-none text-gray-300"
               value={owner} onChange={(e) => updateTask({ owner: e.target.value })} 
            />
         </div>
         <div className="flex items-center bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
            <Folder className="w-3.5 h-3.5 text-gray-500 mr-2 shrink-0" />
            <input 
               type="text" placeholder="Repository..."
               className="w-full text-xs bg-transparent focus:outline-none text-gray-300"
               value={repo} onChange={(e) => updateTask({ repo: e.target.value })} 
            />
         </div>
         <div className="flex items-center bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
         ...
         <div className="flex items-start bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
            ...
            <textarea 
               ...
               value={body} onChange={(e) => updateTask({ body: e.target.value })} 
            />
         </div>
         <button 
          ...
          {result && (
            <button
              onClick={() => {
                ...
                a.download = 'GitHubNode-result.json';
                a.click();
              }}
