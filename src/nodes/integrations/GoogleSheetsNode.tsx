...
import { Maximize, FileSpreadsheet, FileText, Table, Key, Database, AlertCircle, Download, Loader2 } from 'lucide-react';
import { createResourceNode } from '../shared/BaseResourceNode';

const GoogleSheetsNodeBody = ({ task, updateTask }: any) => {
  const [isFetching, setIsFetching] = useState(false);
  const [error, setError] = useState('');
  const [result, setResult] = useState('');
  const [apiKey, setApiKey] = useState(() => localStorage.getItem('gsheets_api_key') || '');
  
  const [spreadsheetId, set_spreadsheetId] = useState(() => localStorage.getItem('gsheets_spreadsheet_id') || '');
  
  const actionType = task.actionType || 'read';
  const spreadsheetIdTask = task.spreadsheetId || '';
  ...
  const handleFetch = async () => {
    ...
    try {
      const res = await fetch('/api/integrations/gsheets', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: actionType, apiKey, spreadsheetId, range, data }),
      });
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            localStorage.setItem('gsheets_api_key', e.target.value);
          }}
        />
      </div>

      <div className="flex items-center gap-2 bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
        <FileSpreadsheet className="w-3.5 h-3.5 text-gray-500 shrink-0" />
        <input
          type="text"
          ...
          value={spreadsheetId}
          onChange={(e) => {
            set_spreadsheetId(e.target.value);
            localStorage.setItem('gsheets_spreadsheet_id', e.target.value);
          }}
        />
      </div>
      ...
      <div className="flex flex-wrap gap-2 text-xs">
         ...
         <button 
            onClick={() => updateTask({ actionType: 'append' })}
            className={`flex-1 min-w-[80px] flex items-center justify-center py-1.5 rounded-lg border transition-colors ${actionType === 'append' ? 'bg-[#2a2b36] text-white border-[#3f3f46]' : 'bg-[#13141c] text-gray-400 border-transparent hover:bg-[#1a1b23]'}`}
         >
            append
         </button>
      </div>
      ...
      <div className="flex flex-col gap-2">
         <div className="flex items-center bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
            ...
            <input 
               type="text" placeholder="Range (e.g. Sheet1!A1:D10)..."
               className="w-full text-xs bg-transparent focus:outline-none text-gray-300"
          ...
          {result && (
            <button
              onClick={() => {
                ...
                a.download = 'GoogleSheetsNode-result.json';
                a.click();
              }}
