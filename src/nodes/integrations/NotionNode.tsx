...
const NotionNodeBody = ({ task, updateTask }: any) => {
  ...
  const actionType = task.actionType || 'Search Pages';
  ...
  const handleFetch = async () => {
    ...
    try {
      const res = await fetch('/api/integrations/notion', {
        ...
        body: JSON.stringify({ action: actionType, apiKey, query, title, content, databaseId }),
      });
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2 text-xs">
         <button 
            onClick={() => updateTask({ actionType: 'Search Pages' })}
            className={`flex-1 min-w-[80px] flex items-center justify-center py-1.5 rounded-lg border transition-colors ${actionType === 'Search Pages' ? 'bg-[#2a2b36] text-white border-[#3f3f46]' : 'bg-[#13141c] text-gray-400 border-transparent hover:bg-[#1a1b23]'}`}
         >
            Search Pages
         </button>
         <button 
            onClick={() => updateTask({ actionType: 'Create Page' })}
            className={`flex-1 min-w-[80px] flex items-center justify-center py-1.5 rounded-lg border transition-colors ${actionType === 'Create Page' ? 'bg-[#2a2b36] text-white border-[#3f3f46]' : 'bg-[#13141c] text-gray-400 border-transparent hover:bg-[#1a1b23]'}`}
         >
            Create Page
         </button>
         <button 
            onClick={() => updateTask({ actionType: 'Read Database' })}
            className={`flex-1 min-w-[80px] flex items-center justify-center py-1.5 rounded-lg border transition-colors ${actionType === 'Read Database' ? 'bg-[#2a2b36] text-white border-[#3f3f46]' : 'bg-[#13141c] text-gray-400 border-transparent hover:bg-[#1a1b23]'}`}
         >
            Read Database
         </button>
      </div>
      ...
      <div className="flex flex-col gap-2">
         ...
         <div className="flex items-center bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
            ...
            <input 
               type="text" placeholder="Database ID..."
               className="w-full text-xs bg-transparent focus:outline-none text-gray-300"
               value={databaseId} onChange={(e) => updateTask({ databaseId: e.target.value })} 
            />
         </div>
         <button 
          ...
          {result && (
            <button
              onClick={() => {
                ...
                a.download = 'NotionNode-result.json';
                a.click();
              }}
