...
const TrelloNodeBody = ({ task, updateTask }: any) => {
  ...
  const [apiKey, setApiKey] = useState(() => localStorage.getItem('trello_api_key') || '');
  
  const [token, set_token] = useState(() => localStorage.getItem('trello_token') || '');
  
  const actionType = task.actionType || 'boards';
  const tokenTask = task.token || '';
  ...
  const handleFetch = async () => {
    ...
    try {
      const res = await fetch('/api/integrations/trello', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: actionType, apiKey, token, boardId, listId, name, desc }),
      });
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            localStorage.setItem('trello_api_key', e.target.value);
          }}
        />
      </div>
      ...
      <div className="flex items-center gap-2 bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            localStorage.setItem('trello_token', e.target.value);
          }}
        />
      </div>
      ...
      <div className="flex flex-wrap gap-2 text-xs">
         <button 
            onClick={() => updateTask({ actionType: 'boards' })}
            className={`flex-1 min-w-[80px] flex items-center justify-center py-1.5 rounded-lg border transition-colors ${actionType === 'boards' ? 'bg-[#2a2b36] text-white border-[#3f3f46]' : 'bg-[#13141c] text-gray-400 border-transparent hover:bg-[#1a1b23]'}`}
         >
            boards
         </button>
         <button 
            onClick={() => updateTask({ actionType: 'lists' })}
            className={`flex-1 min-w-[80px] flex items-center justify-center py-1.5 rounded-lg border transition-colors ${actionType === 'lists' ? 'bg-[#2a2b36] text-white border-[#3f3f46]' : 'bg-[#13141c] text-gray-400 border-transparent hover:bg-[#1a1b23]'}`}
         >
            lists
         </button>
         <button 
            onClick={() => updateTask({ actionType: 'cards' })}
            className={`flex-1 min-w-[80px] flex items-center justify-center py-1.5 rounded-lg border transition-colors ${actionType === 'cards' ? 'bg-[#2a2b36] text-white border-[#3f3f46]' : 'bg-[#13141c] text-gray-400 border-transparent hover:bg-[#1a1b23]'}`}
         >
            cards
         </button>
         <button 
      ...
      <div className="flex flex-col gap-2">
         <div className="flex items-center bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
            <Layout className="w-3.5 h-3.5 text-gray-500 mr-2 shrink-0" />
            <input 
               type="text" placeholder="Board ID..."
               className="w-full text-xs bg-transparent focus:outline-none text-gray-300"
               value={boardId} onChange={(e) => updateTask({ boardId: e.target.value })} 
            />
         </div>
         <div className="flex items-center bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
            <List className="w-3.5 h-3.5 text-gray-500 mr-2 shrink-0" />
            <input 
               type="text" placeholder="List ID..."
               className="w-full text-xs bg-transparent focus:outline-none text-gray-300"
               value={listId} onChange={(e) => updateTask({ listId: e.target.value })} 
            />
         </div>
         <div className="flex items-center bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
            ...
            <input 
               ...
               className="w-full text-xs bg-transparent focus:outline-none text-gray-300"
               value={name} onChange={(e) => updateTask({ name: e.target.value })} 
            />
         </div>
         <div className="flex items-start bg-[#13141c] border border-[#2a2b36] rounded-lg p-2 focus-within:border-cyan-500 transition-colors">
            ...
            <textarea 
               ...
               value={desc} onChange={(e) => updateTask({ desc: e.target.value })} 
            />
         </div>
         <button 
          ...
          {result && (
            <button
              onClick={() => {
                ...
                a.download = 'TrelloNode-result.json';
                a.click();
              }}
