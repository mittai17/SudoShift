...
import { Youtube, FileText, Download, Sparkles, Languages, Users, Clock, AlignLeft, CheckSquare, Map, Key, Loader2, AlertCircle } from 'lucide-react';
...
const YoutubeTranscribeBody = ({ task, updateTask }: any) => {
  ...
  useEffect(() => {
    if (apiKey) {
      localStorage.setItem('transcriptapi_key', apiKey);
    }
  }, [apiKey]);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={handleTranscribe} disabled={isTranscribing || !url || !apiKey}
          className="p-2 bg-cyan-500 hover:bg-cyan-600 text-white rounded-lg transition-colors font-bold disabled:opacity-50 text-xs flex items-center gap-1"
        >
      ...
      <div className="bg-[#13141c] border border-[#2a2b36] rounded-lg overflow-hidden focus-within:border-cyan-500 transition-colors">
        <div className="bg-[#1a1b23] text-[10px] text-gray-400 uppercase tracking-wider font-bold px-3 py-1.5 border-b border-[#2a2b36] flex items-center justify-between">
          ...
          {transcript && (
            <button
              ...
              className="hover:text-white" title="Export Transcript"
            >
        ...
        <textarea
          ...
          placeholder={isTranscribing ? 'Extracting transcript...' : 'Transcript will appear here...'}
        ...
        <div className="bg-[#2a2b36]/20 border border-[#2a2b36] rounded-xl p-2 space-y-2">
          <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1 flex items-center justify-between">
            ...
            <button
              ...
              className="hover:text-white text-gray-500" title="Export Notes"
            >
              <Download className="w-3 h-3" />
            </button>
          </div>
