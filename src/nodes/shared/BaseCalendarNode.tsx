...
export function createCalendarNode(config: CalendarConfig) {
  const CalendarComponent = ({ data: _ }: { data: any }) => {
            ...
            <div className="grid grid-cols-7 gap-0.5">
              ...
              {cells.map((day, i) => (
                <button key={i} onClick={() => day && setSelected(day)}
                  disabled={!day}
                  className={`text-center text-xs py-1 rounded transition-colors ${!day ? '' : day === selected && month === today.getMonth() && year === today.getFullYear() ? 'text-white font-bold' : day === today.getDate() && month === today.getMonth() && year === today.getFullYear() ? 'font-bold text-gray-800 bg-gray-100' : 'text-gray-600 hover:bg-gray-100'}`}
                  style={day === selected ? { backgroundColor: config.accentColor } : {}}>
                  {day || ''}
                </button>
              ))}
            </div>
