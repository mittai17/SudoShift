...
export interface GoalNodeConfig {
  label: string;
  accentColor: string;
...
export function createGoalNode(config: GoalNodeConfig, BodyComponent: React.FC<{ task: TaskData, updateTask: (u: Partial<TaskData>) => void }>) {
  const GoalComponent = ({ data, selected }: { data: any, selected?: boolean }) => {
    const { task } = data;
        ...
        if (subtasks.length > 0) {
          ...
          if (currentNode) {
            const newNodes = subtasks.map((st, i) => {
              ...
              return {
                id: newId,
                type: 'goalProjectNode', // default sub-item for goal
                position: { 
                  x: currentNode.position.x + 350, 
                  y: currentNode.position.y + (i * 120) - ((subtasks.length - 1) * 60)
                },
                data: {
                  task: { id: newId, title: st, description: '', matrix: 'GOAL_PROJECT', deadline: null }
                }
              };
            });
    ...
    const handleQuickAdd = () => {
      ...
      const newNode = {
        ...
        data: { task: { id: newId, title: 'New Project', description: '', matrix: 'GOAL_PROJECT', deadline: null } }
      };
    ...
    return (
      <NodeWrapper data={data} selected={selected} defaultColor={config.accentColor}>
        ...
        <div className={`flex flex-col ${config.width || 'w-80'} rounded-2xl shadow-xl bg-[#1a1b23] border ${selected ? 'border-[#a855f7] shadow-[#a855f7]/20' : 'border-[#2a2b36] hover:border-gray-600'} transition-all group overflow-hidden`} style={{ width: task.width, height: task.height }}>
          ...
          {/* Left Handle with Plus */}
          <Handle type="target" position={Position.Left} className="w-4 h-4 bg-[#2a2b36] border-2 border-[#1a1b23] hover:w-6 hover:h-6 hover:bg-[#a855f7] transition-all -ml-2 z-10 flex items-center justify-center">
          ...
          {/* Body content injected per goal type */}
          <div className="p-4 space-y-4">
            <textarea
              className="w-full text-sm text-gray-300 bg-[#13141c] border border-[#2a2b36] rounded-xl p-3 focus:outline-none focus:border-[#a855f7] resize-none min-h-[80px] placeholder-gray-600 transition-colors"
          ...
          {/* Right Handle with Plus */}
          <Handle type="source" position={Position.Right} className="w-4 h-4 bg-[#2a2b36] border-2 border-[#1a1b23] hover:w-6 hover:h-6 hover:bg-[#a855f7] transition-all -mr-2 z-10 flex items-center justify-center">
  ...
  GoalComponent.displayName = `GoalNode_${config.label.replace(/\s/g, '')}`;
