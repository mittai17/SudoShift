...
export interface HabitNodeConfig {
  label: string;
  accentColor: string;
...
export function createHabitNode(config: HabitNodeConfig, BodyComponent: React.FC<{ task: TaskData, updateTask: (u: Partial<TaskData>) => void }>) {
  const HabitComponent = ({ data, selected }: { data: any, selected?: boolean }) => {
    const { task } = data;
          ...
          if (currentNode) {
            const newNodes = subtasks.map((st, i) => {
              ...
              return {
                ...
                data: {
                  task: { id: newId, title: st, description: '', matrix: 'HABIT_NOTE', deadline: null }
                }
              };
            });
    ...
    const handleQuickAdd = () => {
      ...
      const newNode = {
        ...
        data: { task: { id: newId, title: 'Habit Log', description: '', matrix: 'HABIT_NOTE', deadline: null } }
      };
      const newEdge = { id: `edge-${nodeId}-${newId}`, source: nodeId!, target: newId, type: 'labeled-edge', data: { label: 'Log' } };
    ...
    return (
      <NodeWrapper data={data} selected={selected}>
        ...
        <div className={`flex flex-col ${config.width || 'w-80'} rounded-2xl shadow-xl bg-[#1a1b23] border ${selected ? 'border-orange-500 shadow-orange-500/20' : 'border-[#2a2b36] hover:border-gray-600'} transition-all group overflow-hidden`} style={{ width: task.width, height: task.height }}>
          
          <Handle type="target" position={Position.Left} className="w-4 h-4 bg-[#2a2b36] border-2 border-[#1a1b23] hover:w-6 hover:h-6 hover:bg-orange-500 transition-all -ml-2 z-10 flex items-center justify-center">
          ...
          <div className="p-4 space-y-4">
            <textarea
              className="w-full text-sm text-gray-300 bg-[#13141c] border border-[#2a2b36] rounded-xl p-3 focus:outline-none focus:border-orange-500 resize-none min-h-[80px] placeholder-gray-600 transition-colors"
          ...
          <Handle type="source" position={Position.Right} className="w-4 h-4 bg-[#2a2b36] border-2 border-[#1a1b23] hover:w-6 hover:h-6 hover:bg-orange-500 transition-all -mr-2 z-10 flex items-center justify-center">
  ...
  HabitComponent.displayName = `HabitNode_${config.label.replace(/\s/g, '')}`;
