...
export function createTaskNode(config: TaskNodeConfig, BodyComponent: React.FC<{ task: TaskData, updateTask: (u: Partial<TaskData>) => void }>) {
  const TaskComponent = ({ data, selected }: { data: any, selected?: boolean }) => {
    const { task } = data;
        ...
        if (subtasks.length > 0) {
          ...
          if (currentNode) {
            const newNodes = subtasks.map((st, i) => {
              ...
              return {
                id: newId,
                type: 'taskChecklistNode',
                position: { 
                  x: currentNode.position.x + 350, 
                  y: currentNode.position.y + (i * 120) - ((subtasks.length - 1) * 60)
                },
                data: {
                  task: { id: newId, title: st, description: '', matrix: 'TASK_CHECKLIST', deadline: null }
                }
              };
            });
    ...
    const handleQuickAdd = () => {
      ...
      const newNode = {
        ...
        data: { task: { id: newId, title: 'New Subtask', description: '', matrix: 'TASK_CHECKLIST', deadline: null } }
      };
      const newEdge = { id: `edge-${nodeId}-${newId}`, source: nodeId!, target: newId, type: 'labeled-edge', data: { label: 'Depends On' } };
    ...
    return (
      <NodeWrapper data={data} selected={selected}>
        ...
        <div className={`flex flex-col ${config.width || 'w-80'} rounded-2xl shadow-xl bg-[#1a1b23] border ${selected ? 'border-emerald-500 shadow-emerald-500/20' : 'border-[#2a2b36] hover:border-gray-600'} transition-all group overflow-hidden`} style={{ width: task.width, height: task.height }}>
          
          <Handle type="target" position={Position.Left} className="w-4 h-4 bg-[#2a2b36] border-2 border-[#1a1b23] hover:w-6 hover:h-6 hover:bg-emerald-500 transition-all -ml-2 z-10 flex items-center justify-center">
          ...
          <div className="p-4 space-y-4">
            <textarea
              className="w-full text-sm text-gray-300 bg-[#13141c] border border-[#2a2b36] rounded-xl p-3 focus:outline-none focus:border-emerald-500 resize-none min-h-[80px] placeholder-gray-600 transition-colors"
          ...
          <Handle type="source" position={Position.Right} className="w-4 h-4 bg-[#2a2b36] border-2 border-[#1a1b23] hover:w-6 hover:h-6 hover:bg-emerald-500 transition-all -mr-2 z-10 flex items-center justify-center">
  ...
  TaskComponent.displayName = `TaskNode_${config.label.replace(/\s/g, '')}`;
