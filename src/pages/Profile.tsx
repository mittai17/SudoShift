...
export default function Profile() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-gray-900 font-sans selection:bg-[#6366f1] selection:text-white pb-20 relative overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
            ...
            <div className="md:col-span-5 space-y-6">
              ...
              {/* Profile Card */}
              <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-6 text-center relative overflow-hidden flex flex-col items-center">
                <div className="absolute top-0 inset-x-0 h-24 bg-gradient-to-r from-indigo-500/10 to-violet-500/10 border-b border-slate-100" />
                ...
                <h2 className="text-xl font-bold text-slate-800 mt-4 max-w-full truncate">{fullName}</h2>
                <p className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-full mt-1.5">@{username}</p>

                {/* Statistics Grid */}
                <div className="w-full grid grid-cols-3 gap-3 border-t border-slate-100 pt-6 mt-6">
                  <div className="bg-slate-50 rounded-2xl p-2.5 border border-slate-150/50 flex flex-col items-center">
                    <Layers className="w-4 h-4 text-indigo-600 mb-1" />
                    <span className="text-sm font-extrabold text-slate-800 leading-none">{stats.workspaces}</span>
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wide mt-1">Spaces</span>
                  </div>

                  <div className="bg-slate-50 rounded-2xl p-2.5 border border-slate-150/50 flex flex-col items-center">
                    <Folder className="w-4 h-4 text-indigo-600 mb-1" />
                    <span className="text-sm font-extrabold text-slate-800 leading-none">{stats.folders}</span>
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wide mt-1">Folders</span>
                  </div>

                  <div className="bg-slate-50 rounded-2xl p-2.5 border border-slate-150/50 flex flex-col items-center">
                    <LayoutDashboard className="w-4 h-4 text-indigo-600 mb-1" />
                    <span className="text-sm font-extrabold text-slate-800 leading-none">{stats.canvases}</span>
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wide mt-1">Canvases</span>
                  </div>
                </div>
              </div>

              {/* ID Metadata Card */}
              <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-5 space-y-3">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Collaborator Details</span>
                <div className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl border border-slate-200/50">
                  <div className="min-w-0 flex-1 pr-3">
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">User Unique ID</span>
                    <p className="text-[10px] font-semibold text-slate-500 truncate mt-0.5">{user?.id}</p>
                  </div>
                  <button 
                    onClick={copyUserId}
                    className="p-2 bg-white hover:bg-slate-100 text-slate-500 hover:text-slate-700 rounded-xl transition-all border border-slate-200 shrink-0 shadow-sm active:scale-95"
                    title="Copy User ID"
                  >
            ...
            <div className="md:col-span-7 bg-white border border-slate-200 rounded-3xl shadow-xl p-6 md:p-8 space-y-6">
              <div>
                <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Collaborator Settings</h1>
              ...
              <form onSubmit={handleUpdateProfile} className="space-y-5">
                ...
                <label className="block">
                  ...
                  <div className="relative">
                    <input
                      ...
                      type="email"
                      value={user?.email || ''}
                      className="w-full bg-slate-50 border border-slate-200 text-slate-400 rounded-2xl pl-10 pr-4 py-2.5 text-sm cursor-not-allowed font-medium"
                    />
                    <Lock className="w-4 h-4 text-slate-350 absolute left-3.5 top-1/2 -translate-y-1/2" />
                  </div>
                  <span className="text-[10px] text-slate-400 mt-1 block">Authentication credentials are managed via Supabase.</span>
                </label>
                ...
                <label className="block">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full bg-white border rounded-2xl pl-10 pr-4 py-2.5 text-sm focus:outline-none transition-all font-semibold text-slate-800 ${
                    ...
                    />
                    <span className="text-slate-400 font-bold text-sm absolute left-3.5 top-1/2 -translate-y-1/2 select-none">@</span>
                  </div>
                  <span className="text-[10px] text-slate-400 mt-1 block">Only lower case letters, numbers, underscores, and hyphens allowed. Used for chat @mentions.</span>
                </label>
                ...
                <label className="block">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-white border border-slate-200 rounded-2xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all text-slate-800 font-semibold"
                    />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-5 sm:grid-cols-10 gap-2.5 p-4 bg-slate-50 rounded-2xl border border-slate-200/50">
                    {PALETTE.map(color => {
                ...
                <button
                  ...
                  className="w-full bg-[#6366f1] hover:bg-[#4f46e5] active:scale-98 disabled:opacity-60 disabled:scale-100 text-white py-3 rounded-2xl text-sm font-bold flex items-center justify-center gap-2 transition-all shadow-md hover:shadow-lg disabled:cursor-not-allowed"
                >
                  {saving ? (
                    <>
